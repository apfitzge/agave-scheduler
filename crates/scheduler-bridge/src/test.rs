...
use agave_scheduler_bindings::{
    ProgressMessage, SharablePubkeys, SharableTransactionBatchRegion, SharableTransactionRegion,
    TpuToPackMessage, TransactionResponseRegion, WorkerToPackMessage, pack_message_flags,
    processed_codes,
};
use agave_scheduling_utils::handshake::server::{AgaveSession, Server};
use agave_scheduling_utils::handshake::{ClientLogon, client};
use agave_scheduling_utils::responses_region::{
...
use crate::{KeyedTransactionMeta, ScheduleBatch, SchedulerBindingsBridge, TransactionKey};
    ...
    M: Copy,
{
    ...
    pub fn new(worker_count: usize, worker_req_cap: usize) -> Self {
        assert!(worker_req_cap.is_power_of_two(), "shaq requires power of 2 queue sizes");
        ...
        let fds = files.into_iter().map(IntoRawFd::into_raw_fd).collect();
        let client_session = client::setup_session(&logon, fds).unwrap();

        Self { bridge: SchedulerBindingsBridge::new(client_session), agave }
    }
    ...
    pub fn queue_tpu(&mut self, tx: &VersionedTransaction) {
        ...
        let msg = TpuToPackMessage {
            transaction: SharableTransactionRegion { offset, length: serialized.len() as u32 },
            flags: 0,
            src_addr: [0; 16],
        };
    ...
    pub fn queue_check_response(
        ...
        if let Some(keys) = keys {
            ...
            response.resolved_pubkeys = SharablePubkeys { offset, num_pubkeys: keys.len() as u32 };
        }
    ...
    pub fn pop_schedule(&mut self) -> Option<ScheduleBatch<Vec<KeyedTransactionMeta<M>>>> {
        for (worker_idx, worker) in self.agave.workers.iter_mut().enumerate() {
            ...
            if let Some(msg) = worker.pack_to_worker.try_read() {
                ...
                let batch = unsafe {
                    Batch::<M>::from_sharable_transaction_batch_region(
                        &msg.batch,
    ...
    fn build_single_tx_batch_region(
        ...
        SharableTransactionBatchRegion { num_transactions: 1, transactions_offset: batch_offset }
    }
}
