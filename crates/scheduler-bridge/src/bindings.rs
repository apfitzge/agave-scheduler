...
use agave_scheduler_bindings::worker_message_types::{self, CheckResponse, ExecutionResponse};
...
where
    M: Copy,
{
    const TRANSACTION_BATCH_META_OFFSET: usize = Batch::<M>::TX_META_START;
    const TRANSACTION_BATCH_SIZE: usize = Batch::<M>::TX_META_END;
    ...
    pub fn schedule(
        ...
        // Write the batch.
        queue
            .try_write(PackToWorkerMessage { flags, max_working_slot, batch })
...
impl TransactionState {
    ...
    /// intentionally omits:
    ///
    /// - **Reserved account key demotion**: reserved accounts (sysvars,
    ///   builtins) in writable positions are not demoted to read-only.
    /// - **Program account demotion**: writable program accounts are not
    ///   demoted when `bpf_loader_upgradeable` is absent.
    ///
